<template>
  <div class="export-img-dialog">
    <div class="configs">
      <div class="row">
        <div class="title">{{ t('exportFormat') }}：</div>
      ...
      <div class="row">
        ...
        <el-radio-group class="config-item" v-model="rangeType">
          <el-radio-button style="width: 50%;" value="all" label="all">{{ t('allPages') }}</el-radio-button>
          <el-radio-button style="width: 50%;" value="current" label="current">{{ t('currentPage') }}</el-radio-button>
          <!-- <el-radio-button style="width: 33.33%;" value="custom" label="custom">自定义</el-radio-button> -->
        </el-radio-group>
      </div>
      <div class="row" v-if="rangeType === 'custom'">
        <div class="title" :data-range="`（${range[0]} ~ ${range[1]}）`">{{ t('selfRange') }}：</div>
        <el-slider class="config-item" range :min="1" :max="templates.length" :step="1" v-model="range"/>
      </div>

      <div class="row">
        <div class="title">{{ t('imageQuality') }}：</div>
        <el-slider class="config-item" :min="0" :max="1" :step="0.1" v-model="quality"/>
      </div>

      <div class="row">
        <div class="title">{{ t('imageResolution') }}：</div>
      ...
      <div class="row">
        ...
        <div class="title">{{ t('ignoreBloods') }}：</div>
        <div class="config-item">
    ...
    <div class="btns">
      <el-button class="btn export" type="primary" @click="downloaImage()" v-loading.fullscreen.lock="Exporting">{{ t('exportImages') }}</el-button>
