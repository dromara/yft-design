...
<template>
   <el-dialog v-model="dialogVisible" :width="500" :title="$t('gpt.openGPTTitle')"  class="upload-dialog"  :before-close="handleClose" :close-on-click-modal="false">
    <div>
      <div class="mb-[20px] text-center">办级变中级，中级变高级，让前端开发更轻松高效!</div>
      <div class="cursor-pointer text-center text-[18px] font-bold" @click="handleCopy">点击复制微信号</div>
    </div>
    <template #footer>
...
const handleCopy = debounce(async function() {
  const result = await copyText('微信号')
